...
extern bool Key_left, Key_right, Key_up, Key_fire;
...
void CZoneCEView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	{
		if (y<(CY+16)) {moveup=!moveup; if (moveup) movedown=false;}
		//if (y>(CY+32)) {movedown=!movedown; if (movedown) moveup=false;}
		if ((y>(CY+16)) && (y<(CY+32)) && (x<(CX+16))) {moveright=!moveright; if (moveright) moveleft=false;}
		if ((y>(CY+16)) && (y<(CY+32)) && (x>(CX+32))) {moveleft=!moveleft; if (moveleft) moveright=false;}
		if ((y>(CY+16)) && (y<(CY+32)) && (x>(CX+16)) && (x<(CX+32))) 
