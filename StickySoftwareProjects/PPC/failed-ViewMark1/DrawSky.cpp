...
short int FindClipCode(int x, int y)
{
	short int code=0;
		...
		if ((z>0) && (z<pi)) // i.e. not behind observer
		{
			...
			if (fail==0) // Object visible
			{
				...
				if (!flags[InvertOn])
				{
					...
					if ((!starname)&&(zodiac)) pDC->SetTextColor(RGB(0,0,0));
				}
				else
				{
					pDC->SetBkColor(WHITENESSNESS);
...
void DrawMarkers(CDC *pDC)
{
	...
	for (short int i=0;i<8;i++)
	{
		...
		//if ((z<90) || (z>270))
		{
			...
			{
				case 0: {mess.Format(L"N\0");break;}
				case 1: {mess.Format(L"NE\0");break;}
				case 2: {mess.Format(L"E\0");break;}
				case 3: {mess.Format(L"SE\0");break;}
				case 4: {mess.Format(L"S\0");break;}
				case 5: {mess.Format(L"SW\0");break;}
				case 6: {mess.Format(L"W\0");break;}
				case 7: {mess.Format(L"NW\0");break;}
			}
				if (!flags[InvertOn])
					{
						pDC->SetBkColor(BLACKNESSNESS);
