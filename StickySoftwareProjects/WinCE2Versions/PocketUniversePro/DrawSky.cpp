...
//	AfxMessageBox(L"MG2",MB_OK,0);


	// like tools?
	pMemMoonDC.CreateCompatibleDC(pDC);
	lunar_bitmap.LoadBitmap(MAKEINTRESOURCE(IDB_BITMAP_MOON));
	pMemMoonDC.SelectObject(&lunar_bitmap);

//	AfxMessageBox(L"MG3",MB_OK,0);
...
void PlotStar(CDC *pDC,int x,int y,short int mg)
{
	...
	pDC->BitBlt(x-4,y-4,9,9,&pMemDC_star[mg],0,0,SRCCOPY);

//	pDC->BitBlt(x-4,y-4,9,9,&pMemDC_star[mg],0,0,SRCPAINT/*SRCCOPY*/);
}
