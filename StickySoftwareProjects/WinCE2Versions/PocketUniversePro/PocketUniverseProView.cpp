...
void CPocketUniverseProView::OnMenuSettimedate() 
{
	...
	adlg.m_daylight=local_daylight;
	...
	if (reply==IDOK)
	{
		...
		local_daylight=adlg.m_daylight;
...
void CPocketUniverseProView::OnMenuDisplay() 
{
	MessageBox(L"Plop",L"Plop",MB_OK);
	CClientDC aDC(this);
	MessageBox(L"Plop",L"1",MB_OK);
	update=true;
	makegrid(&aDC);
	MessageBox(L"Plop",L"2",MB_OK);
	DrawSky(&aDC);
	MessageBox(L"Plop",L"3",MB_OK);
