...
BEGIN_MESSAGE_MAP(CZoneCEView, CView)
	...
	ON_COMMAND(ID_MENU_FILL, OnMenuFill)
	ON_UPDATE_COMMAND_UI(ID_MENU_FILL, OnUpdateMenuFill)
	ON_COMMAND(ID_MENU_SIZE, OnMenuSize)
	ON_UPDATE_COMMAND_UI(ID_MENU_SIZE, OnUpdateMenuSize)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CZoneCEView::OnUpdateMenuFill(CCmdUI* pCmdUI) 
{
		pCmdUI->SetCheck(!Fill);
}

void CZoneCEView::OnMenuSize() 
{
	size=!size;
	CClientDC dc(this);
	ChangeMode(&dc);
...
void CZoneCEView::OnUpdateMenuSize(CCmdUI* pCmdUI) 
{
	pCmdUI->SetCheck(size);
}
