...
#define BLUENESSNESS RGB(0,0,128)
...
void UsePen(CDC *pDC, int pen)
{
	...
	if (flags[InvertOn])
	{
		...
		case 6:pDC->SelectObject(DKGreyPen);break;
		case 5:pDC->SelectObject(LTGreyPen);break;
...
void DefinePens(bool contrast)
{
	...
	HorizonBrush.CreateSolidBrush(RGB(192,192,192));
	...
	Yellow.CreatePen(PS_SOLID,1,RGB(255,255,0)); // lt grey

	HorizonBrush.CreateSolidBrush(RGB(0,128,0)); // green
...
/*
	...
	WhitePen.CreatePen(PS_SOLID,1,RGB(255,0,0));
	LTGreyPen.CreatePen(PS_SOLID,1,RGB(255,0,0));
	DKGreyPen.CreatePen(PS_SOLID,1,RGB(192,0,0));
	BlackPen.CreatePen(PS_SOLID,1,RGB(0,0,0));

	Blue.CreatePen(PS_SOLID,1,RGB(92,0,0));	// black
	DarkGreen.CreatePen(PS_SOLID,1,RGB(92,0,0));	// dk grey
	Red.CreatePen(PS_SOLID,1,RGB(255,0,0)); // redish??
	Cyan.CreatePen(PS_SOLID,1,RGB(128,0,0));
	Yellow.CreatePen(PS_SOLID,1,RGB(128,0,0)); // lt grey

	HorizonBrush.CreateSolidBrush(RGB(128,0,0)); // green
...
void makegrid(CDC *pDC)
{
	...
	lunar_bitmap.LoadBitmap(IDB_BITMAP11);
	pMemMoonDC.SelectObject(&lunar_bitmap);

	DefinePens(flags[HighContrast]);
...
void DrawMoon(CDC *pDC,int x, int y, bool venus)
{
	...
	if ((flags[GfxMoon]) && (!venus))
		pDC->StretchBlt(x-(size)+1,y-(size)+1,size*2-1,size*2-1,&pMemMoonDC,0,0, 80, 80, SRCPAINT /*SRCCOPY*/); 
			...
			if (fail==0) // Object visible
			{
				...
				*/
				...
				if (!flags[InvertOn])
				{
					...
					if (!starname) pDC->SetTextColor(RGB(255,255,0));
					if ((!starname)&&(zodiac)) pDC->SetTextColor(RGB(255,0,0));
				}
				else
				{
					pDC->SetBkColor(WHITENESSNESS);
...
void DrawHiddenSky (CDC *pDC)
{
	...
	if (!flags[InvertOn])
		if (flags[HighContrast])
			...
			pDC->FillSolidRect(ORIGIN,ORIGIN,ORIGIN+WIDTH+4,ORIGIN+HEIGHT+4,BLUENESSNESS); 
	else
