...
CBitmap /*control_bitmap,control_bitmap2,*/background_bitmap;
CDC /*pMemDC,pMemDC2,*/pMemDC3;
...
void ClipAndDrawLine(CDC *pDC, float x0, float y0, float x1, float y1, short int ink);
...
// Program pens...
	...
	CString s;
	CRect brect(545,147,625,167); // score
	CRect arect(545,192,625,212); // paused // lives
	CRect crect(545,236,625,256); // High
	CRect drect(545,283,625,303); // Ammo
	CPen DKGreyPen,LTGreyPen,ThickPen;
	CPen BlackPen(PS_SOLID,1,RGB(0,0,0));
	CPen CyanPen(PS_SOLID,1,RGB(0,128,128));
	CPen WhitePen(PS_SOLID,0,RGB(255,255,255));
	CPen BluePen;


//	HPEN BlackPen = CreatePen(PS_SOLID,1,RGB(255,0,0));
//	HPEN CyanPen = CreatePen(PS_SOLID,1,RGB(0,0,0));
...
void AddL(int x1, int y1, int x2, int y2, short int col)
{
...
//	linecol[linemode][linecount[linemode]]=col;
//	linec[linecount]=c;
	linecount[linemode]++;
}
...
void DrawL(CDC *pDC)
{
	...
	if (linecount[linemode]==0) return;
...
//		col=linecol[linemode][i];
/*
		if (col!=ocol)
		{
			switch (col) {
			case 0:{pDC->SelectObject(&BlackPen);pDC->FillSolidRect(0,0,1,1,BLACKRGB);};break;
			case 1:{pDC->SelectObject(&DKGreyPen);pDC->FillSolidRect(0,0,1,1,RGB(82,82,82));};break;
			case 2:{pDC->SelectObject(&LTGreyPen);pDC->FillSolidRect(0,0,1,1,RGB(190,190,190));};break;
			case 4:{pDC->SelectObject(&BluePen);pDC->FillSolidRect(0,0,1,1,RGB(0,0,255));};break;
			default:{pDC->SelectObject(&WhitePen);pDC->FillSolidRect(0,0,1,1,WHITERGB);};break;
			}
...
void EraseL(CDC *pDC)
{
	...
	pDC->FillSolidRect(0,0,1,1,BLUEGREEN);
	//pDC->SelectObject(&WhitePen);
	//GetStockObject(BLACK_PEN);
	short int x1,y1,x2,y2,lx,ly;
	...
	if (linecount[nlinemode]==0) return;
...
void Message2(CDC *pDC)
{
	...
	for (int x=0;x<9;x++)
	{
		PrintLetter(message[x],80+x*12,80,pDC);
	}
}
...
void Message4(CDC *pDC)
{
	...
	for (int x=0;x<9;x++)
	{
		PrintLetter(message[x],60+16+x*20,80,pDC);
	}
}
...
void Message5(CDC *pDC)
{
	...
	for (int x=0;x<21;x++)
	{
		PrintLetter(message[x],60+x*10,80,pDC);
	}
}
...
//		PrintLetter(s[x],ORIGIN+160+x*20,168,pDC);
		PrintLetter(s[x],60+16+x*20,80,pDC);
	}
...
void Message7(CDC *pDC, int i)
{
	...
	int l;
	switch(i) {
		case 0:strcpy(message,"   TANKZONE PRO ");l=15;break;
		...
		case 10:strcpy(message,"   COPYRIGHT 1999");l=17;break;
		case 11:strcpy(message,"WWW STICKY CO UK ");l=17;break;
	}

	tx=3;ty=5;
	for (int x=0;x<l;x++) // was 
		PrintLetter(message[x],4+(4*(l/2))+18*x,45,pDC);
...
//	sndPlaySound((const unsigned short *)pSound1, SND_MEMORY|SND_ASYNC);

//	return;


//	sndPlaySound((const unsigned short *)IDR_WAVE1, SND_MEMORY|SND_ASYNC);
...
Windscreen::Windscreen()
{
	for (int i=0;i<57;i++)
	{
		broken_glass[i].x=14*smash[i].x;
		broken_glass[i].y=16+24*smash[i].y;
	}
}
...
//	pDC->SelectObject(&DKGreyPen);
	...
	for (int i=0;i<36;i++)
	{
		...
		if ((z1>1) && (z2>1))
		{
			...
			ClipAndDrawLine(pDC,px1,-20+py1,px2,-20+py2,1);		
		}
	}
	AddL(ORIGINX,6+ORIGIN+HALFHEIGHT,ORIGINX+WIDTH,6+ORIGIN+HALFHEIGHT,1);		// central line?
}
...
void Cube::Draw(CDC *pDC)
{
	...
	if (frame==0) 
		if ((x1*x1+z1*z1)<900) pDC->FillSolidRect(RX+x1,RY-z1,3,3,BRIGHTBLUE);
	...
	for (int i=0;i<12;i++)
	{
		...
		if ((z1>1) && (z2>1))
		{
			...
			if (!((px1<-HALFWIDTH) && (px2<-HALFWIDTH)
				||(px1>HALFWIDTH) && (px2>HALFWIDTH)))
				ClipAndDrawLine(pDC,px1,py1,px2,py2,4);
		}
	}
}
...
void Flag::Draw(CDC *pDC)
{
	...
	if (frame==0)
		if ((x1*x1+z1*z1)<900) pDC->FillSolidRect(RX+x1,RY-z1,3,3,BRIGHTGREEN);
	...
	{
		x1=p1[i].x+position.x;y1=p1[i].y+position.y;z1=p1[i].z+position.z;
		x2=p1[i+1].x+position.x;y2=p1[i+1].y+position.y;z2=p1[i+1].z+position.z;
		...
		if ((z1>1)&&(z2>1))
		{
			...
			//{
				if (!((px1<-HALFWIDTH) && (px2<-HALFWIDTH)
					||(px1>HALFWIDTH) && (px2>HALFWIDTH)))
					ClipAndDrawLine(pDC,px1,py1,px2,py2,3);
					px2=px1;py2=py1;
			//}
		}
	}
...
//	pDC->SelectObject(&BlackPen);
	...
	if (frame==0)
		pDC->FillSolidRect(RX+x1,RY-z1,3,3,BRIGHTYELLOW);
		//pDC->Ellipse(RX+x1,RY-z1,(RX+3)+x1,(RY+3)-z1);
...
void Wipe(CDC *pDC)
{
	...
	// fill plop
	pDC->FillSolidRect(ORIGINX,ORIGIN,WIDTH,HEIGHT,BLUEGREEN/*BLACKRGBBLUEGREEN*/);
...
void SmallWipe(CDC *pDC)
{
	pDC->SelectObject(&WhitePen);
	pDC->SelectStockObject(WHITE_BRUSH);
	pDC->FillSolidRect(ORIGINX,ORIGIN,WIDTH,120,BLUEGREEN);
...
void ClipAndDrawLine(CDC *pDC, float dx0, float dy0, float dx1, float dy1,short ink)
{
	...
	if (accept) 
		AddL(x0+ORIGINX+HALFWIDTH,y0+ORIGIN+HALFHEIGHT,x1+ORIGINX+HALFWIDTH,y1+ORIGIN+HALFHEIGHT,ink);
}
...
void DrawMoon(CDC *pDC)
{
	...
	AddL(mx+4,my+0,mx+1,my+15,1);
	AddL(mx+1,my+15,mx+6,my+25,1);
	AddL(mx+6,my+25,mx+15,my+29,1);
	AddL(mx+15,my+29,mx+28,my+27,1);
	AddL(mx+28,my+27,mx+12,my+17,1);
	AddL(mx+12,my+17,mx+4,my+0,1);
}
...
void InitSystem(CDC *pDC)
{
	...
	//LOGPEN CyanData={PS_SOLID,0,RGB(255,0,0)};

	//CyanPen.CreatePenIndirect(&CyanData);

	BluePen.CreatePen(PS_SOLID,0,RGB(0,0,80));
	//CyanPen.CreatePen(PS_SOLID,0,BLUEGREEN);
	//WhitePen.CreatePen(PS_SOLID,0,WHITERGB);
	LTGreyPen.CreatePen(PS_SOLID,0,LTGREYRGB);
	DKGreyPen.CreatePen(PS_SOLID,0,DKGREYRGB);
	//BlackPen.CreatePen(PS_SOLID,0,BLACKRGB);
	ThickPen.CreatePen(PS_SOLID,3,BLACKRGB);
	GameState=1;
	...
	pMemDC3.CreateCompatibleDC(pDC);
	background_bitmap.LoadBitmap(IDB_SCREEN_BITMAP);
	pMemDC3.SelectObject(&background_bitmap);

	// plop up
	pDC->BitBlt(0,0,640,480,&pMemDC3,0,0,SRCCOPY);

}

void DrawBackground(CDC *pDC, bool wipe)
{
	pDC->BitBlt(0,0,640,480,&pMemDC3,0,0,SRCCOPY);
...
void InitLevel(CDC *pDC)
{
	...
	moveup=false;movedown=false;moveleft=false;moveright=false;Key_fire=false;

	switch(Level) {
		case 1: EnemyCount=6;Saucer1=false;Tank1=true;Tank2=false;AmmoBonus=4;TankSpeed=.2;break;
		case 2: EnemyCount=2;Saucer1=true;Tank1=false;Tank2=false;AmmoBonus=3;SaucerSpeed=.3;break;
		case 3: EnemyCount=8;Saucer1=false;Tank1=true;Tank2=true;AmmoBonus=3;TankSpeed=.2;break;
		...
		case 6: EnemyCount=12;Saucer1=false;Tank1=true;Tank2=true;AmmoBonus=3;TankSpeed=.3;break;
...
void DrawRadar(CDC *pDC)
{
	...
	//WaitCommEvent(NULL,(LPDWORD)EV_POWER,NULL);
...
/* temporarily
  ...
  */
	...
	pDC->BitBlt(543,24,72,72,&pMemDC3,543,24,SRCCOPY);
	...
	pDC->SetTextColor(WHITERGB);
	pDC->SetBkColor(BLUEGREEN); // doesn't change circle, only writing
	...
	s.Format (L"%5d",Score);
	pDC->DrawText(s, brect, DT_NOCLIP | DT_CENTER | DT_SINGLELINE);

	s.Format (L"%5d",High);
	pDC->DrawText(s, crect, DT_NOCLIP |DT_CENTER | DT_SINGLELINE);

	s.Format (L"%5d ",Lives);
	pDC->DrawText(s, arect, DT_NOCLIP |DT_CENTER | DT_SINGLELINE);
	...
	pDC->DrawText(s, drect, DT_NOCLIP |DT_CENTER | DT_SINGLELINE);
}
...
void SpinPlayer(CDC *pDC, float angle)
{
	...
	tank1shell.Rotate(-angle);	// Moving in opposite dition!
	moon_position-=angle;
...
void Redraw(CDC *pDC)
{
	...
	// Sites
		...
		{
			AddL(ORIGINX+HALFWIDTH-15,ORIGIN+HALFHEIGHT-15,ORIGINX+HALFWIDTH-5,ORIGIN+HALFHEIGHT-5,3);
			AddL(ORIGINX+HALFWIDTH+15,ORIGIN+HALFHEIGHT+15,ORIGINX+HALFWIDTH+5,ORIGIN+HALFHEIGHT+5,3);
			AddL(ORIGINX+HALFWIDTH+15,ORIGIN+HALFHEIGHT-15,ORIGINX+HALFWIDTH+5,ORIGIN+HALFHEIGHT-5,3);
			AddL(ORIGINX+HALFWIDTH-15,ORIGIN+HALFHEIGHT+15,ORIGINX+HALFWIDTH-5,ORIGIN+HALFHEIGHT+5,3);
		}
		...
		{
			AddL(ORIGINX+HALFWIDTH-20,ORIGIN+HALFHEIGHT-20,ORIGINX+HALFWIDTH-15,ORIGIN+HALFHEIGHT-15,2);
			AddL(ORIGINX+HALFWIDTH+20,ORIGIN+HALFHEIGHT+20,ORIGINX+HALFWIDTH+15,ORIGIN+HALFHEIGHT+15,2);
			AddL(ORIGINX+HALFWIDTH+20,ORIGIN+HALFHEIGHT-20,ORIGINX+HALFWIDTH+15,ORIGIN+HALFHEIGHT-15,2);
			AddL(ORIGINX+HALFWIDTH-20,ORIGIN+HALFHEIGHT+20,ORIGINX+HALFWIDTH-15,ORIGIN+HALFHEIGHT+15,2);
		}
