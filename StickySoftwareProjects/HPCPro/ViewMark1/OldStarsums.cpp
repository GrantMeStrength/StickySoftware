...
void ConvertAltAztoRaDec(double azm, double alt, double *ra, double *dec)
{
	...
	//printf("\nP=");printDMS(R2D(p));printf("  Q=");printDMS(R2D(q));printf("\n");
...
void MakeLocal(double RA,double Dec, double *Az, double *Alt)
{
	...
	*Az=(p);
	*Alt=(q);
	//*Az=atan2(sin(D2R(HR)*15),cos(D2R(HR)*15)*SinLat-tan(D2R(Dec))*CosLat);
	//*Az=atan2(sin(D2R(HR)*15)*cos(D2R(Dec)),SinLat*cos(D2R(Dec))*cos(D2R(HR)*15)-CosLat*sin(D2R(Dec)));
