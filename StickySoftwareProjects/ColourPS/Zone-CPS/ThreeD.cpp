...
bool grey=false;//true;
short int ORIGINX=23;
#define ORIGIN 98

//#define CY 16
//#define CX 180

#define HEIGHT 167//319// 320 // was 192
#define HALFHEIGHT 83//160
#define TOOCLOSE 3
#define HIT 0.8// 0.75
//#define	MAP 6
//#define HARDER 2000
#define moon_limit 12//34 // was 20

#define WHITERGB (RGB(255,255,255))
#define BLACKRGB (RGB(0,0,0))
#define LTGREYRGB (RGB(190,190,190))
#define DKGREYRGB (RGB(120,80,80))
#define BLUEGREEN (RGB(0,182,170))
...
#define WIDTH 196//239//480
#define HALFWIDTH 98//120
...
// Program pens...
	...
	CString s;
	CRect brect(96,23-4,162,37-4); // score
	CRect arect(96,37-4,162,51-4); // paused // lives
	CRect crect(96,51-4,162,65-4); // High
	CRect drect(96,65-4,162,79-4); // Ammo
	CPen WhitePen,DKGreyPen,LTGreyPen,BlackPen,ThickPen,CyanPen,BluePen;
...
void DrawColourDot(CDC *pDC, int x, int y, int col)
{
	switch (col) {
	case 0:	pDC->FillSolidRect(x,y,3,3,BRIGHTBLUE);break;
	case 1:	pDC->FillSolidRect(x,y,3,3,BRIGHTRED);break;
	case 2:	pDC->FillSolidRect(x,y,3,3,BRIGHTYELLOW);break;
	case 3:	pDC->FillSolidRect(x,y,3,3,BRIGHTGREEN);break;
	}
}

void DrawGreyDot(CDC *pDC, int x, int y)
{
	pDC->FillSolidRect(x,y,3,3,WHITERGB);
}

void DrawDot(CDC *pDC, int x, int y, int col)
{
	if (grey) DrawGreyDot(pDC,x,y);
	else DrawColourDot(pDC,x,y,col);
}
...
void DrawColourL(CDC *pDC)
{
	...
	for (i=0;i<linecount[linemode];i++)
	{
		...
		if (col!=ocol)
		{
			...
			case 4:pDC->SelectObject(&BluePen);break;
			default:pDC->SelectObject(&WhitePen);break;
			}
...
void Message1(CDC *pDC)
{
	...
	for (int x=0;x<9;x++)
	{
		PrintLetter(message[x],10+x*20,40,pDC);
	}
...
void Message2(CDC *pDC)
{
	...
	for (int x=0;x<9;x++)
	{
		PrintLetter(message[x],10+x*20,160,pDC);
	}
}
...
void Message4(CDC *pDC)
{
	...
	for (int x=0;x<9;x++)
	{
		PrintLetter(message[x],10+x*20,80,pDC);
	}
}
...
void Message5(CDC *pDC)
{
	...
	for (int x=0;x<21;x++)
	{
		PrintLetter(message[x],10+x*18,80,pDC);
	}
}
...
//		PrintLetter(s[x],ORIGIN+160+x*20,168,pDC);
		PrintLetter(s[x],20+x*20,80,pDC);
	}
...
void Message7(CDC *pDC, int i)
{
	...
	for (int x=0;x<l;x++) // was 
		PrintLetter(message[x],-4+(3*(l/2))+8*x,45,pDC);
...
//	sndPlaySound((const unsigned short *)SHOOT, SND_MEMORY|SND_ASYNC);
...
Windscreen::Windscreen()
{
	for (int i=0;i<57;i++)
	{
		broken_glass[i].x=7*smash[i].x;
		broken_glass[i].y=16+12*smash[i].y;
	}
}
...
void Cube::Draw(CDC *pDC)
{
	...
	if (frame==0) 
		if ((x1*x1+z1*z1)<900) DrawDot(pDC,RX+x1,RY-z1,0); // pDC->FillSolidRect(RX+x1,RY-z1,3,3,BRIGHTBLUE);
...
void Flag::Draw(CDC *pDC)
{
	...
	if (frame==0)
		if ((x1*x1+z1*z1)<900) DrawDot(pDC,RX+x1,RY-z1,3);
			//pDC->FillSolidRect(RX+x1,RY-z1,3,3,BRIGHTGREEN);
...
void Wipe(CDC *pDC)
{
	...
	pDC->FillSolidRect(ORIGINX,ORIGIN,WIDTH,HEIGHT,BLUEGREEN);
...
void InitSystem(CDC *pDC)
{
	...
	BluePen.CreatePen(PS_SOLID,1,RGB(0,0,80));
	CyanPen.CreatePen(PS_SOLID,1,BLUEGREEN);
...
/* temporarily
  ...
  */
	...
	pDC->BitBlt(5,8,72,72,&pMemDC3,5,8,SRCCOPY);
