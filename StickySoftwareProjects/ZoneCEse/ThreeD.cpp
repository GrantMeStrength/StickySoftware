...
#define HEIGHT 192
#define WIDTH 480 // 640 //480
#define HALFWIDTH 240 // 320 //240
...
//		PrintLetter(message[x],ORIGIN+150+x*20,80,pDC);
		PrintLetter(message[x],ORIGIN+HALFWIDTH-60+x*20,80,pDC);
...
//		PrintLetter(s[x],ORIGIN+130+x*20,150,pDC);
		PrintLetter(s[x],ORIGIN+HALFWIDTH-220+x*20,150,pDC);
...
void Message7(CDC *pDC, int i)
{
	...
	int l;
	switch(i) {
		case 0:strcpy(message,"TANKZONE SE");l=11;break;
...
Windscreen::Windscreen()
{
	for (int i=0;i<57;i++)
	{
		//broken_glass[i].x=16*smash[i].x; // MADE WIDER!

		broken_glass[i].x=((22*smash[i].x));
...
void Windscreen::Draw(CDC *pDC)
{
	...
	for (int i=0;i<57;i++)
	{
		...
		{
			if (mode==false)
				pDC->LineTo(broken_glass[i].x+ORIGIN-20,broken_glass[i].y+ORIGIN);
			else
			{
				pDC->MoveTo(broken_glass[i].x+ORIGIN-20,broken_glass[i].y+ORIGIN);
				mode=false;
			}
...
//	pDC->Rectangle(ORIGIN+x1+39,ORIGIN-z1+39,ORIGIN+x1+41,ORIGIN-z1+41);
	...
	pDC->FillSolidRect(ORIGIN+x1+39,ORIGIN-z1+39,2,2,RGB(80,80,80));



//	pDC->FillSolidRect(ORIGIN,ORIGIN,ORIGIN+WIDTH+4,ORIGIN+HEIGHT+4,RGB(0,0,0)); 
...
//	pDC->Rectangle(ORIGIN+x1+39,ORIGIN-z1+41,ORIGIN+x1+41,ORIGIN-z1+40);
	pDC->FillSolidRect(ORIGIN+x1+39,ORIGIN-z1+41,2,2,RGB(80,80,80));
...
void Attract2(CDC *pDC)
{
	...
	pDC->FillSolidRect(ORIGIN,ORIGIN,WIDTH,HEIGHT,RGB(255,255,255));
...
void GameOver2(CDC *pDC)
{
	...
	pDC->SelectStockObject(LTGRAY_BRUSH);
	//pDC->Rectangle(ORIGIN,ORIGIN,ORIGIN+WIDTH,ORIGIN+HEIGHT);
	pDC->FillSolidRect(ORIGIN,ORIGIN,WIDTH,HEIGHT,RGB(190,190,190));

	pDC->SelectObject(&DKGreyPen);
	pDC->SelectStockObject(WHITE_BRUSH);
	pDC->FillSolidRect(ORIGIN+50,ORIGIN+30,WIDTH-100,HEIGHT-60,RGB(255,255,255));
...
void InitSystem(CDC *pDC)
{
	...
	hidden_bitmap.CreateCompatibleBitmap(pDC,WIDTH,240);



	/*
	hidden_bitmap.CreateCompatibleBitmap(pDC,100,100);
	pDC->SelectObject(&hidden_bitmap);
	pDC->SelectObject(&WhitePen);
	pDC->SelectStockObject(WHITE_BRUSH);
	pDC->Rectangle(0,0,90,90);
	pDC->LineTo(0,0);
	pDC->MoveTo(90,90);

	dcHide.CreateCompatibleDC(pDC);
	dcHide.SelectObject(&hidden_bitmap);
	*/
...
void ClearScreen1(CDC *pDC)
{
	...
	{	
		...
		pDC->FillSolidRect(ORIGIN,ORIGIN,WIDTH+2,HALFHEIGHT,RGB(255,255,255));

		pDC->SelectStockObject(LTGRAY_BRUSH);
		//pDC->Rectangle(ORIGIN,ORIGIN+HALFHEIGHT,ORIGIN+WIDTH+2,ORIGIN+HEIGHT+2);
		pDC->FillSolidRect(ORIGIN,ORIGIN+HALFHEIGHT,WIDTH+2,HEIGHT+2,RGB(190,190,190));
...
//		pDC->Rectangle(ORIGIN,ORIGIN,ORIGIN+WIDTH+2,ORIGIN+HEIGHT+2);
		pDC->FillSolidRect(ORIGIN,ORIGIN,WIDTH+2,HEIGHT+2,RGB(255,255,255));
...
void RedrawNoFlicker(CDC *pDC)
{
	...
	pDC->BitBlt(0,0,(WIDTH-1),239,&pMemDC,0,0,SRCCOPY);
